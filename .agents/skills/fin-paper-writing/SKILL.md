...
每章节完成后，必须执行以下 AI 味和底气检测：

**AI 味检测清单**（扫描到任一关键词即为不合格）：

扫描关键词（中文）：值得注意的是 | 综上所述 | 从某种意义上 | 客观来说 | 在一定程度上 | 某种程度上 | 相对而言 | 近年来 | 随着时代发展 | 当今社会 | 可能存在 | 有待进一步研究 | 首先我们需要 | 非常重要的 | 具有重大意义的 | 已经被 | 可以被认为 | XX研究表明

**底气指数评估**（结论段必须满足≥1项）：

| 底气要素 | 检测标准 |
|----------|----------|
| 具体数字 | β = X.XXX，95% CI [...]，p = X |
| 经济规模 | 占均值/标准差的 X% |
| 机制描述 | 通过 X 渠道，而非 Y 渠道 |
| 对比发现 | A 效应是 B 效应的 X 倍 |
| 理论对应 | 与 [理论名] 的预测一致/相反 |

**表格引导句检查**：每张表格前是否有≥2句引导（主题句 + 来源句 + 核心发现句），每张表格后是否有≥1句解读。

**图表注释解读检查**：每个图表的 caption/注释是否包含"说明什么"（而不仅是"展示什么"），是否包含具体数字或对比描述。

**语言质量检查不通过的章节，不得进入 Review 环节，必须先重写。**
...
在 Phase C（Orchestration）中，每章节写作完成后，必须触发语言质量 Review：
...
| 检查项 | 标准 | 不通过处置 |
|--------|------|-----------|
| AI 味关键词 | 全文出现 ≤ 0 次 | 重写对应段落 |
| 底气指数 | 结论段满足 ≥ 1 项 | 重写结论段 |
| 表格引导句 | 每表前后 ≥ 3 句 | 补充引导句 |
| 图表注释 | 每图 ≥ 1 句解读 | 补充注释 |
| 段落数 | 符合 PAPER_OUTLINE.md 下限 | 扩充或合并 |

### T08：CHAPTER_STATUS.md 语言质量字段

更新 CHAPTER_STATUS.md 模板，增加语言质量追踪字段：
...
| 版本 | 日期 | 主要变更 | 语言质量 | 作者 |
|------|------|---------|---------|------|
| draft_v1 | 2026-06-13 | 初始草稿 | AI味0/底气足 | Agent |
```
