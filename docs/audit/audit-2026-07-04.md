...
| #95 | PR-5: audit trail (本文) | docs/audit/audit-2026-07-04.md; README badge | `4d9f317` |
| #96 | PR-6: 推覆盖率 +3pp (import smoke) | tests/test_module_imports.py + fail-under 25→28 | `9be4d4e` |
| #97 | PR-7A: 真实 deep tests (econometrics_extended) | tests/test_econometrics_extended.py — 75 tests / 9 skips; 9%→54.5% per-file | _pending_ |
| (待) | PR-7B: 真实 deep tests (survival/panel_cointegration/panel_var/time_varying) | +~200 tests | _pending_ |
| (待) | PR-7C: 真实 deep tests (factor_models/interactive_explorer/empirical_agent) | +~150 tests | _pending_ |
| (待) | PR-7D: 真实 deep tests (agent_pipeline/knowledge_graph/research_rag) | +~150 tests — CORE | _pending_ |
| (待) | PR-7E: 真实 deep tests (volatility/data_validator/data_fetcher/observability) | +~150 tests | _pending_ |
| (待) | PR-7F: 真实 deep tests (core/ modules) + audit_guard fix + mypy cleanup | +~150 tests; 60%+ target | _pending_ |
...
| 覆盖率 fail-under | 30% (虚胖) | 25% (真实, PR-1) → 28% (PR-6 import smoke) → 30% (PR-7A 真实 tests) → 60% (PR-7F) | 更诚实 |
| 依赖上界 | 0/51 | 51/51 | +100% |
