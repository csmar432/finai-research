...
| #96 | PR-6: 推覆盖率 +3pp (import smoke) | tests/test_module_imports.py + fail-under 25→28 | _pending_ |
| (待) | PR-8: 60% 覆盖 (真实 deep tests) | ~425 测试函数推到 60% — **deferred**（一个 PR 不能写 425 测试） | _pending_ |
...
| 覆盖率 fail-under | 30% (虚胖) | 25% (真实, PR-1) → 28% (PR-6 import smoke) → 60% (PR-8 deferred) | 更诚实 |
| 依赖上界 | 0/51 | 51/51 | +100% |
