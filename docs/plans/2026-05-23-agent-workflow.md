...
> **文档状态：** 历史规划文档（2026-05-23 编写，2026-06-01 更新）
> **重要提示：** 本文档描述旧架构（2026-06-04 重构前）。当前代码请参考 `scripts/agent_pipeline.py` 和 `scripts/research_framework/`。
> 文中提及的 `paper_full_pipeline` 等脚本已重构或重命名，详见 [CLAUDE.md](../../CLAUDE.md)。
...
| P3-8 | tool_selector.py TOOL_REGISTRY包含所有37个MCP服务器 | `tool_selector.py` | 注册表已扩展 | ✅ 已验证正确 |
## 十、待办清单
...
E — 第七波全面核查（已完成，2026-05-29 下午批次）
  ...
  ⚠️ E8: us_esg_formatter.py/us_esg_regression.py 硬编码路径 — $USER路径（移植性问题）
...
E2 — MCP服务器详细审计（2026-05-29，5个并行subagent）→ 已全部修复
  ...
  Python层
    ...
    ⚠ us_esg_formatter.py/us_esg_regression.py硬编码$USER路径（E8）
