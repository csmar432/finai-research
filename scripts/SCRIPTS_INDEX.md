...
| [🗑️ Obsolete](#四-obsolete-废弃) | 2 | 已废弃，不应使用 |
| **合计** | **73** | |
...
| `research_workflow.py` | — | **已废弃** → 使用 `agent_pipeline.py` |
...
| `econometrics.py` | `python scripts/econometrics.py` | 核心计量引擎（DID / OLS / PSM / GMM / Heckman 等） |
...
| `report_generator.py` | `python scripts/report_generator.py` | 研报生成器（LaTeX + Word 双格式） |
...
| `review_layer.py` | 680 | `ReviewLayer`, `batch_review()`, `fix_quality()` | `paper_write`, `agent_pipeline`, `research_workflow`, `paper_full_pipeline` |
...
| `fetch_msci_cyh.py` | 数据采集 | 陈宇浩 MSCI ESG 评级抓取 |
| `fetch_msci_xu.py` | 数据采集 | 许哲逸 MSCI ESG 评级抓取 |
...
| 脚本 | 废弃原因 | 替代品 |
|------|----------|--------|
| `research_workflow.py` | 在 docstring 中明确标注 deprecated | `agent_pipeline.py`（完全替代） |

以下脚本是个人数据采集脚本，应根据需要保留或清理：

| 脚本 | 说明 |
|------|------|
| `fetch_msci_xu.py` | 许哲逸个人 MSCI ESG 数据抓取脚本 |
| `fetch_msci_xu2.py` | 前者的修订版（无独立入口，可能是过期版本） |
| `fetch_msci_cyh.py` | 陈宇浩个人 MSCI ESG 数据抓取脚本 |
...
| 目录 | 内容 |
|------|------|
| `scripts/core/` | 核心 Agent 系统（17个模块）：orchestrator, memory, planner, reflector, session, tool_selector, hitl_gate, llm_gateway, latex_lint, latex_diff, pdf_vision_check, sandbox, self_evolution 等 |
| `scripts/research_framework/` | 实证研究框架（16个模块）：data_fetcher, regression_engine, modern_did, synthetic_control, synthetic_did, rdd, spatial_regression, iv_panel, robustness_runner, panel_quantile_regression, interactive_fixed_effects, local_projections_did, triple_diff_did, vuong_kob, leamer_sensitivity, diagnostic_reporter |
| `scripts/research_directions/` | 研究方向定义（6个）：digital_finance, green_finance, macro_finance, asset_pricing, corporate_finance, carbon_economics |
...
*本索引由 `scripts/SCRIPTS_INDEX.md` 维护，最后更新: 2026-06-04*
