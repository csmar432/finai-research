#!/usr/bin/env python3
"""
从新浪财经ESG API批量获取股票的MSCI评级。

用法：
    python scripts/fetch_msci_xu.py --person "许哲逸" --excel /path/to/input.xlsx --output /path/to/output.xlsx

环境变量：
    EXCEL_PATH   — 输入Excel文件路径
    OUTPUT_PATH  — 输出Excel文件路径
    CACHE_DIR    — 缓存目录（默认: data/）
    PERSON_NAME  — Excel中负责人的列值（默认: "许哲逸"）
"""
import argparse
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15, context=ctx) as response:
                try:
                    ...
                    _log.warning(f"JSON decode error for {symbol}: {e}")
                    return {'symbol': symbol, 'msci_rating': None, 'msci_date': None}
...
def main():
    parser = argparse.ArgumentParser(description="批量获取股票的MSCI ESG评级")
    parser.add_argument('--person', default=os.environ.get('PERSON_NAME', '许哲逸'),
                        help='Excel中负责人的列值（环境变量: PERSON_NAME）')
    parser.add_argument('--excel', help='输入Excel路径（环境变量: EXCEL_PATH）')
    parser.add_argument('--output', help='输出Excel路径（环境变量: OUTPUT_PATH）')
    args = parser.parse_args()

    global _EXCEL_PATH, _OUTPUT_PATH
    _EXCEL_PATH = args.excel if args.excel else os.environ.get('EXCEL_PATH', DEFAULT_EXCEL_PATH)
    _OUTPUT_PATH = args.output if args.output else os.environ.get('OUTPUT_PATH', DEFAULT_OUTPUT_PATH)
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        if row[6] == person:
            ...
            target_row_indices.append(row_idx)
