...
% TODO (researcher-supplied): Replace the 3 placeholder case studies below
% with full case studies that include actual numerical results (point
% estimates, standard errors, sample sizes, robustness checks).
%
% A complete case study should be ~1 page and contain:
%   1. Research question and motivation
%   2. Identification strategy chosen and why
%   3. Data sources (with citations) and sample construction
%   4. Main regression result table (3-5 columns, 5-8 rows)
%   5. At least one robustness check result
%   6. Heterogeneity analysis (if applicable)
%   7. Researcher interpretation
%
% Source for full case-study templates: papers/finai_methodology/case_study_template.md
% (to be created when this paper is finalized for submission).

\subsection{Case Study 1: Carbon Trading and Green Innovation}

We apply FinAI to the research question: \textit{Does China's carbon emission trading scheme (ETS) affect firms' green innovation?}

FinAI generates a complete research design including: (i) staggered DID with Callaway-Sant'Anna estimator; (ii) 47-paper literature review via OpenAlex MCP; (iii) 12 robustness checks including Borusyak imputation and Goodman-Bacon decomposition; (iv) JF/JFE-format LaTeX manuscript outline.

Total pipeline time: 25 minutes. The system correctly identifies the need for staggered DID due to the multi-cohort ETS rollout (2011/2013/2021 pilots) and recommends appropriate estimators.

% TODO: Add Table 5.1 with main DID coefficient, standard errors,
% sample size, and fixed effects specification. Provide reference for
% a published paper using similar identification (e.g., Hu et al. 2023).

\subsection{Case Study 2: Green Credit Policy and Firm TFP}

We apply FinAI to the question: \textit{Does the 2012 Green Credit Guidelines affect TFP of heavy-pollution firms?}

FinAI produces a feasibility report assessing 5 data sources (A-share financials, green patents, pollution lists, city GDP), identifies 5 critical risks with mitigation strategies, and recommends a PSM-DID design. Data feasibility rating: medium.

% TODO: Add Table 5.2 with PSM-DID coefficient, control variables,
% sample before/after matching, ATT for sub-samples.

\subsection{Case Study 3: Digital Finance and ESG Performance}

We apply FinAI to the question: \textit{How does digital finance penetration affect corporate ESG performance?}

FinAI identifies a Panel IV strategy using the PKU Digital Finance Index as the treatment variable and internet penetration as the instrument. The novelty check correctly identifies this as a gap in the JF/JFE literature (novelty score: 7.2/10).

% TODO: Add Table 5.3 with 2SLS first-stage F-statistic, second-stage
% coefficient, Kleibergen-Paap rk Wald F statistic, and overidentification
% test results.
